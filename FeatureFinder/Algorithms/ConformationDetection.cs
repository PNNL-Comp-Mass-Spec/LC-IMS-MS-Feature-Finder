...
namespace FeatureFinder.Algorithms
{
	public static class ConformationDetection
	{
		...
		public static IEnumerable<LCIMSMSFeature> DetectConformationsForLCIMSMSFeature(LCIMSMSFeature lcimsmsFeature)
		{
			...
			driftProfileXYPairList = PadXYPairsWithZeros(driftProfileXYPairList, globalDriftTimeMinimum, globalDriftTimeMaximum);
			...
			for (double i = globalDriftTimeMinimum; i <= globalDriftTimeMaximum; i += DRIFT_TIME_SLICE_WIDTH)
			{
				...
				if (intensity > previousIntensity)
				{
					...
					if (!movingUp)
					{
						xyPairList = PadXYPairsWithZeros(xyPairList, driftTimeMinimum, i - DRIFT_TIME_SLICE_WIDTH);
			...
			xyPairList = PadXYPairsWithZeros(xyPairList, driftTimeMinimum, globalDriftTimeMaximum);
		...
		*/ 
		...
		private static List<XYPair> PadXYPairsWithZeros(List<XYPair> driftProfileXYPairList, double globalDriftTimeMinimum, double globalDriftTimeMaximum)
		{
			for (int i = 1; i <= 5; i++)
			{
				double lowDriftTime = globalDriftTimeMinimum - (DRIFT_TIME_SLICE_WIDTH * i);
				double highDriftTime = globalDriftTimeMaximum + (DRIFT_TIME_SLICE_WIDTH * i);
